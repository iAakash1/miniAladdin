...
| **MODEL** | YELLOW | Registry: 69 experimental, 34 retired, **0 validated/promoted**. The 21-session portfolio improved validation net Sharpe in 10/10 seeds but is validation evidence with fold heterogeneity. The rich-data question (EXP-011) is prepared, not run. | EXP-011 result; a frozen candidate plus explicit holdout authorisation. |
...
4. EXP-011 has not been run (by design; the owner runs it).
5. Playwright E2E and Python `pip-audit` not run.
